...
const { bundle, canCloseStream } = require('../../../dist/src/cmds/build/bundle');
...
describe('bundle', () => {
  ...
  describe('canCloseStream', () => {
    ...
    it('writes to console error if there is a bundle Error', async () => {
      const writeErrorMock = jest.spyOn(miscUtils, 'writeError').mockImplementation(() => {
        throw new Error('error message');
      });
      await expect(canCloseStream({ bundleError: true })).rejects.toThrow('error message');
      expect(writeErrorMock).toHaveBeenCalledTimes(1);
    });

    it('console logs if successfully closed bundle stream', async () => {
      const closeStreamMock = jest.spyOn(bundleUtils, 'closeBundleStream').mockImplementation();
      jest.spyOn(miscUtils, 'writeError').mockImplementation();
      const logMock = jest.spyOn(console, 'log').mockImplementation();
      await canCloseStream({ bundleError: false, bundleStream: {}, bundleBuffer: 'foo', src: 'src', dest: 'dest', argv: { stripComments: false } });
    ...
    it('catches error if failed to close bundle stream', async () => {
      ...
      await expect(canCloseStream({ bundleError: false, bundleStream: {}, bundleBuffer: 'foo', src: 'src', dest: 'dest', argv: { stripComments: false } })).rejects.toThrow('error message');
      expect(closeStreamMock).toHaveBeenCalled();
