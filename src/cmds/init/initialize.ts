...
async function buildWeb3Wallet(argv: YargsArgs): Promise<[
  ...
  try {
    const c = await prompt({ question: `Use all default Snap manifest values?`, def: 'yes', shouldClose: false });
  ...
  while (noValidPort) {
    ...
    const inputPort = (await prompt({ question: `local server port:`, def: port.toString(10) }));
    let err, tempPort;
    try {
  ...
  while (invalidDist) {
    // eslint-disable-next-line require-atomic-updates
    dist = await prompt({ question: `output directory:`, def: dist });
    try {
...
async function validateEmptyDir(): Promise<void> {
  ...
  if (existing.length > 0) {
    ...
    const continueInput = await prompt({ question: `Continue?`, def: 'yes' });
